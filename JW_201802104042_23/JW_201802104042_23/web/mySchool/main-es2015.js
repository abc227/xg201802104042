...
let DegreeComponent = class DegreeComponent {
    ...
    delete(degree) {
        ...
        this.degreeService.deleteDegree(degree).subscribe(res => {
            const { message } = res;
...
let DepartmentComponent = class DepartmentComponent {
    ...
    delete(department) {
        this.dataShow = this.dataShow.filter(h => h !== department);
        this.departmentService.deleteDepartment(department).subscribe(res => { alert(res.message); this.router.navigateByUrl('/index/department'); });
    }
};
...
let TeacherService = class TeacherService {
    ...
    getTitle(id) {
        const url = this.titUrl + '?id=' + id;
        return this.http.get(url).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["catchError"])(this.handleError(`getDep id=${id}`)));
    }
    getTea(id) {
        const url = this.teaUrl + '?id=' + id;
        return this.http.get(url).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["catchError"])(this.handleError(`getDep id=${id}`)));
    }
    updateDegree(degree) {
...
let AddTeaComponent = class AddTeaComponent {
    ...
    save(no, name, degree, title, department) {
